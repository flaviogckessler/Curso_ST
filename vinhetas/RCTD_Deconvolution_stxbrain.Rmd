...
Além disso, a paralelização só será possível se você estiver utilizando Linux ou Mac como sistema operacional. Para usuários de Windows, o máximo de cores utilizado terá de ser igual a 1, o que pode aumentar consideravelmente o tempo de processamente. Essa vinheta, diferentemente das demais, possui alguns plots gerados pela análise para que você possa visualizar mesmo que não consiga rodar o fluxo de trabalho integralmente.
...
O conjunto `stxBrain` não precisa ser copiado para o repositório. Ele será instalado pelo `SeuratData` na biblioteca de pacotes do R. O único arquivo de entrada local é `allen_cortex.rds`, que deve ficar em `data/`, ele será instalado na sequência. Os resultados serão gravados em `resultados/`, as figuras em `imagens/`.
...
if (packageVersion("SeuratObject") < package_version("5.0.0")) {
  stop(
    ...
    "Execute novamente a instalação na primeira vinheta e reinicie o R."
  )
}
```
...
O `stxBrain` é distribuído como um pacote de dados separado e também é instalado na primeira vinheta. O próximo bloco somente confirma que o conjunto está disponível antes de continuar.
...
Instale a referência de single-cell com o código abaixo, ele irá salvar o seguinte arquivo: `data/allen_cortex.rds`. **Cuidado:** O arquivo baixado possui cerca de **1GB**.
