...
A instalação de todos os pacotes e de suas dependências está centralizada em `Configuracao_do_ambiente_RStudio.Rmd`. Esta vinheta utiliza a versão 2.2.1 do repositório original do `spacexr` no GitHub, cuja interface oferece `create.RCTD()` e `run.RCTD()` e não é intercambiável com a interface do Bioconductor.
...
Depois de executar a primeira vinheta, reinicie a sessão do R antes de continuar. Isso evita que uma versão do `spacexr` carregada anteriormente permaneça ativa na memória.
