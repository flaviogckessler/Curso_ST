...
1.  explicar por que a deconvolução requer uma referência unicelular anotada;
2.  obter do `SeuratData` as contagens brutas e as coordenadas do Visium;
3.  preparar a referência unicelular do córtex de camundongo do Allen Institute [@Tasic2016];
...
| Referência unicelular | córtex de camundongo do Allen Institute | contagens brutas de genes por células e um rótulo de tipo celular por célula |
...
-   pelo menos 16 GB de memória RAM;
-   acesso à internet para a primeira instalação dos pacotes e dos dados; e
-   de alguns minutos a mais de uma hora para executar o RCTD completo, dependendo do computador e do número de tipos celulares da referência.

A versão HTML desta vinheta pode ser criada sem executar a análise. Para renderizar e executar todo o fluxo, use:

```{r renderizar-vinheta, eval=FALSE}
rmarkdown::render(
  "RCTD_Deconvolution_stxbrain.Rmd",
  params = list(run_analysis = TRUE)
)
```
...
O conjunto `stxBrain` não precisa ser copiado para a pasta da vinheta. Ele será instalado pelo `SeuratData` na biblioteca de pacotes do R. O único arquivo de entrada local é `allen_cortex.rds`, que deve ficar em `data/`. Os resultados serão gravados em `results/`.

``` text
RCTD_Deconvolution_stxbrain/
|-- RCTD_Deconvolution_stxbrain.Rmd
|-- bibliography.bib
|-- data/
|   `-- allen_cortex.rds
...
# Informar manualmente o diretório da vinheta
...
4.  O R mostrará algo semelhante a `[1] "D:/meu_curso/RCTD_Deconvolution_stxbrain"`.
5.  Copie o caminho mostrado entre aspas e substitua o caminho de exemplo no bloco seguinte. No Windows, prefira barras `/`, como no exemplo, ou duplique cada barra invertida: `\\`.
...
diretorio_vinheta <- "/home/flaviok/RCTD_Curso"

diretorio_vinheta <- normalizePath(
  diretorio_vinheta,
  winslash = "/",
  mustWork = TRUE
)
...
diretorio_dados <- file.path(diretorio_vinheta, "data")
diretorio_resultados <- file.path(diretorio_vinheta, "results")
...
if (!file.exists(arquivo_referencia)) {
  stop(
    "A referência unicelular não foi encontrada em: ",
    arquivo_referencia,
...
resumo_genes <- data.frame(
  fonte = c("amostra espacial", "referência unicelular", "interseção"),
  numero_de_genes = c(
...
`create.RCTD()` prepara os dados, seleciona genes informativos e aprende os perfis dos tipos celulares da referência. O servidor utilizado no curso permite trabalhar com 30 núcleos. Em outro computador, ajuste `numero_nucleos` para um valor adequado à máquina disponível.
...
Primeiro, observe onde o RCTD identificou `singlet`, `doublet_certain`, `doublet_uncertain` ou `reject`.
...
1.  Compare os mapas de `first_type` e `second_type`.
2.  Conte quantos spots foram classificados em cada categoria de `spot_class`.
3.  Escolha uma população neuronal e uma população glial e compare suas localizações.
4.  Compare as atribuições do RCTD com estruturas anatômicas visíveis na imagem histológica.

# Solução de problemas

## “Counts must be integers”

Provavelmente foi fornecida uma matriz normalizada. Para a amostra espacial, extraia `layer = "counts"` do ensaio `Spatial`; para a referência, extraia `layer = "counts"` do ensaio `RNA`.

## `create.RCTD()` não foi encontrada

Provavelmente está carregada a versão do Bioconductor. Execute novamente o bloco de instalação, que instala `dmcable/spacexr` pelo GitHub, e reinicie a sessão do R.

## Não foi possível instalar o `stxBrain`

O conjunto é grande e o download pode falhar em uma conexão instável. Execute novamente apenas o bloco **Instalar o conjunto de dados**. Se o erro persistir, confirme que o R possui acesso à internet e que existe espaço em disco na biblioteca de pacotes.

## Há poucas células de referência para um tipo celular

Por padrão, o RCTD exige pelo menos 25 células por tipo. Una subclasses raras apenas quando isso for biologicamente justificável ou remova-as de forma explícita. Não duplique células para aumentar artificialmente o grupo.

## Há poucos genes compartilhados

Confirme que ambos os conjuntos pertencem ao mesmo organismo e usam o mesmo sistema de identificadores gênicos. Se necessário, converta identificadores Ensembl em símbolos gênicos, tratando explicitamente os símbolos duplicados.

## As contagens e as coordenadas não possuem spots em comum

Compare `colnames(contagens_espaciais)` com `rownames(coordenadas_espaciais)`. Não tente alinhar esses objetos apenas pela posição das colunas ou linhas.

## A análise está lenta ou utiliza muita memória

Reduza `numero_nucleos` se o servidor limitar a criação de processos paralelos. Em demonstrações, uma região espacial pode ser analisada separadamente, desde que as limitações biológicas e estatísticas dessa seleção sejam discutidas.
